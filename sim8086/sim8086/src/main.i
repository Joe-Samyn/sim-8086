...
extern int __sigsetjmp_cancel (struct __cancel_jmp_buf_tag __env[1], int __savemask) noexcept (true) __asm__ ("" "__sigsetjmp") __attribute__ ((__returns_twice__));
...
{ "MOV", { { Literal, 0b100010, (8 - (sizeof("100010") - 1)), (sizeof("100010") - 1) }, { D_bit, 0b10, 1, 1 }, { W_bit, 0b1, 0, 1} }},
};
enum Operation {
Op_MOV,
 Op_count
...
Program LoadProgramIntoMemory(std::string filePath)
{
 ...
 if (!file.is_open() || (*__errno_location ()) == 2)
 {
...
void Disassemble(Program &program)
{
 ...
 while (currentByte != program.endAddr)
            ...
            if (entry.bits[0].value == (currentByte >> entry.bits[0].shift))
   {
    ...
    printf("\n");
                usedBits += entry.bits[0].count;
    while (!IsBitsDefined(entry.bits[bitsIndex]))
    {
                    if (usedBits >= 7) currentByte = GetNextByte(cpu.IP);
